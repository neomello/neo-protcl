  ...
  useEffect(() => {
    const handleOrientation = e => {
      if (e.alpha !== null && e.beta !== null && e.gamma !== null) {
        ...
        motionRef.current.alpha =
          motionRef.current.alpha * (1 - smoothing) + (e.alpha || 0) * (Math.PI / 180) * smoothing
        motionRef.current.beta =
          motionRef.current.beta * (1 - smoothing) + (e.beta || 0) * (Math.PI / 180) * smoothing
        motionRef.current.gamma =
          motionRef.current.gamma * (1 - smoothing) + (e.gamma || 0) * (Math.PI / 180) * smoothing
      }
    }
  ...
  useEffect(() => {
      ...
      sortedNodes.forEach(pos => {
        ...
        glowLayers.forEach(layer => {
          ...
          gradient.addColorStop(
            0.5,
            `rgba(${color.r}, ${color.g}, ${color.b}, ${layer.alpha * 0.5})`
          )
        ...
        nodeGradient.addColorStop(
          1,
          `rgba(${color.r * 0.6}, ${color.g * 0.6}, ${color.b * 0.6}, ${baseAlpha * 0.8})`
        )
        ...
        if (isSelected && Math.random() > 0.7) {
          particleSystemRef.current.push(createParticle(projected.x, projected.y, color))
        }
      })
    }
    ...
    const checkNodeHover = (x, y) => {
      ...
      rotatedNodes.forEach(pos => {
        const projected = project3D(pos.x, pos.y, pos.z)
        const dist = Math.sqrt(Math.pow(x - projected.x, 2) + Math.pow(y - projected.y, 2))
